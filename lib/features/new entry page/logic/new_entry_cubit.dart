...
import 'package:pill2/core/service/work_manager_service.dart';
...
class NewEntryCubit extends Cubit<NewEntryState> {
  ...
  Future deleteReminder(
      BuildContext context, AddReminderModel addreminderModel) async {
  ...
  Future addReminder(
    ...
    try {
      ...
      print(" cubit====================>wark manager start run");

      await WorkManagerService().init(addreminderModel);

      print(" cubit====================>wark manager");
  ...
  getReminders() {
    var reminderBox = Hive.box<AddReminderModel>(medcineBox);
    reminderList = reminderBox.values.toList().reversed.toList();
  }
